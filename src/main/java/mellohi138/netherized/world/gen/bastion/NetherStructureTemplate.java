...
public abstract class NetherStructureTemplate extends StructureComponentTemplate {
    ...
    public NetherStructureTemplate() {

    }

    public NetherStructureTemplate(TemplateManager manager, String type, BlockPos pos, Rotation rotation, boolean overWriteIn) {
        super(0);
        this.pieceName = type;
    ...
    public NetherStructureTemplate(TemplateManager manager, String type, BlockPos pos, int distance, Rotation rotation, boolean overwriteIn) {
    ...
    private void loadTemplate(TemplateManager manager) {
        ...
        Template template = manager.getTemplate((MinecraftServer) null, new ResourceLocation(Netherized.MODID, this.templateLocation() + "/" + this.pieceName));
     ...
     */
    public boolean isCollidingExcParent(TemplateManager manager, NetherStructureTemplate parent, List<StructureComponent> structures) {
        List<StructureComponent> collisions = findAllIntersecting(structures);

        boolean foundCollision = false;

        for (StructureComponent collision : collisions) {
            if (((NetherStructureTemplate) collision).getId() != parent.getId()) {
                foundCollision = true;
            }
        }

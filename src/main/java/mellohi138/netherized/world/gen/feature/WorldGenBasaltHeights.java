...
public class WorldGenBasaltHeights extends WorldGenerator {
    ...
    public void intereperetCircle(double x, double y, double z, World world) {
        ...
        for(BlockPos blockPos : affectedConversionPositions) {
            ...
            if(!world.isAirBlock(relPos.down()) && findSurface != 0 && world.getBlockState(relPos).getBlock() == NetherizedBlocks.BASALT) {

                for(int t = 0; t < ModRand.range(1, 7); t++) {
                    world.setBlockState(relPos.add(0,t,0), NetherizedBlocks.BASALT.getDefaultState());
                }
